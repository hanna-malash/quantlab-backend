...
def main() -> int:
    parser = argparse.ArgumentParser(description="QuantLab data ingestion (raw CSV -> normalized CSV).")
    ...
    parser.add_argument("--raw-file", default="", help="Path to raw CSV file (preferred for reproducibility).")
    parser.add_argument("--url", default="", help="Optional URL to download raw CSV.")

    parser.add_argument("--normalized-dir", default="data/normalized", help="Output directory for normalized CSV.")
    args = parser.parse_args()

    provider = CryptoDataDownloadProvider()
    src = CryptoDataDownloadSource(exchange=args.exchange, symbol=args.symbol, timeframe=args.timeframe)
    ...
    output_path = repo.write_bars_csv(bars=bars, symbol=args.symbol, timeframe=args.timeframe)
